...
각 세부 항목에 대해 **0~5점**으로 평가하고, 각 기준별 점수를 합산하여 **최대 15점**을 받습니다. 전체 점수는 **60점 만점**입니다.

1. **심플함과 강렬함**
   - **간결성**: 문구가 불필요한 단어 없이 간결한가?
   - **임팩트**: 첫인상에서 강렬한 인상을 주는가?
   - **명확성**: 메시지가 명확하고 이해하기 쉬운가?

2. **공감과 감성적 연결**
   - **감정 호소력**: 독자의 감정을 자극하고 공감대를 형성하는가?
   - **개인적 연결성**: 개인의 경험이나 감정에 연결될 수 있는가?
   - **보편성**: 다양한 독자층에게 공감될 수 있는 보편적 주제를 담고 있는가?

3. **상상력 자극**
   - **호기심 유발**: 독자의 호기심을 자극하는가?
   - **경험 상상력**: 해당 장소나 경험에 대한 상상을 불러일으키는가?
   - **독창성**: 일반적이지 않은 독특한 접근을 취하고 있는가?

4. **유니크함과 기억에 남는 표현**
   - **차별성**: 다른 카피와 구별되는 독특한 요소가 있는가?
   - **기억 지속성**: 문구가 쉽게 잊혀지지 않고 기억에 남는가?
   - **창의적 표현력**: 신선하고 창의적인 언어 사용을 했는가?

**점수 가이드:**
- **50~60점**: 매우 우수한 카피로, 모든 기준에서 뛰어난 성과를 보입니다.
- **40~49점**: 전반적으로 우수하나, 일부 개선 여지가 있습니다.
- **30~39점**: 보통 수준으로, 몇 가지 주요 개선이 필요합니다.
- **0~29점**: 기준을 충족하지 못하며, 전반적인 개선이 필요합니다.

**응답 형식:**
점수: [0-60 사이의 숫자]

이유: [평가 근거를 구체적으로 작성해 주세요.]

상세점수:
- 심플함과 강렬함: 간결성 [점수]/5, 임팩트 [점수]/5, 명확성 [점수]/5 (합계: [점수]/15)
- 공감과 감성적 연결: 감정 호소력 [점수]/5, 개인적 연결성 [점수]/5, 보편성 [점수]/5 (합계: [점수]/15)
- 상상력 자극: 호기심 유발 [점수]/5, 경험 상상력 [점수]/5, 독창성 [점수]/5 (합계: [점수]/15)
- 유니크함과 기억에 남는 표현: 차별성 [점수]/5, 기억 지속성 [점수]/5, 창의적 표현력 [점수]/5 (합계: [점수]/15)
- 종합 의견: [전체적인 평가 요약과 개선 방향을 제안해 주세요.]
""",
    criteria=[        "심플함과 강렬함",        "공감과 감성적 연결",        "상상력 자극",        "유니크함과 기억에 남는 표현"    ]
)
...
with col1:
    ...
    # 기본 프롬프트 구조
    base_structure = """당신은 사색적이고 감성적인 카피를 창작하는 숙련된 카피라이터입니다. 
아래 제공된 정보를 참고하여 특정 여행지에 대한 관심과 호기심을 유도하는 감성적인 카피를 작성해주세요.

💡 **목표**
1. 독자가 여행지나 특정 경험에 대해 간접적으로 매력을 느끼고 관심을 가지게 만듭니다. 여행지를 모르는 사람을 타겟으로 가정하여 이들을 배려하는 맥락적 배려가 반드시 필요합니다. 
2. 감동적이고 철학적인 메시지를 간결하게 담아내 여행지의 매력을 강조합니다.
3. 한 문장의 카피와 그에 어울리는 짧은 설명을 함께 작성해주세요. 반드시 아래 외에 아무것도 출력하지마세요.
   - **카피**: 여행지나 특정 경험의 정서를 함축한 한 줄의 메시지, 마치 카피라이팅 문구가 전문가 칼럼 블로그의 제목과 공존하는 듯한 느낌 주기.
   - **설명**: 카피의 맥락을 보완하는 짧은 배경 또는 감성적 해설, 반드시 마치 전문가 칼럼 블로그의 내용같이 작성. 메타인지를 활용."""
...
{selected_season}의 특징을 반영합니다."""
    ...
    requirements = """
1. 짧고 강렬한 문장을 작성해주세요.
2. 특정 여행지나 경험이 떠오를 수 있도록 감성적이고 철학적인 관점을 담아주세요.
3. 카피와 설명은 독립적으로도 매력적이어야 하며, 서로의 메시지를 보완해야 합니다.
4. 독자가 여행지의 매력을 간접적으로 경험하고, 호기심을 느끼도록 유도합니다.
"""
